...
import org.juneframework.remoting.RemoteConnectFailureException;
import org.juneframework.remoting.RemoteInvocationFailureException;
import org.juneframework.remoting.RemoteLookupFailureException;
import org.juneframework.remoting.support.RemoteInvocationBasedAccessor;
import org.juneframework.remoting.support.RemoteInvocationUtils;
 ...
 * @see org.juneframework.remoting.rmi.RmiInvocationHandler
...
public class RmiClientInterceptor extends RemoteInvocationBasedAccessor
	...
	public void prepare() throws RemoteLookupFailureException {
		...
		if (this.lookupStubOnStartup) {
			...
			if (logger.isDebugEnabled()) {
				if (remoteObj instanceof org.juneframework.remoting.rmi.RmiInvocationHandler) {
	 ...
	 * {@link org.juneframework.remoting.rmi.RmiClientInterceptorUtils#isConnectFailure}.
	 * @param ex the RMI exception to check
	 * @return whether the exception should be treated as connect failure
	 */
	protected boolean isConnectFailure(RemoteException ex) {
		return org.juneframework.remoting.rmi.RmiClientInterceptorUtils.isConnectFailure(ex);
	}
	...
	protected Object doInvoke(MethodInvocation invocation, Remote stub) throws Throwable {
		if (stub instanceof org.juneframework.remoting.rmi.RmiInvocationHandler) {
			// RMI invoker
			try {
				return doInvoke(invocation, (org.juneframework.remoting.rmi.RmiInvocationHandler) stub);
			}
			catch (RemoteException ex) {
				throw org.juneframework.remoting.rmi.RmiClientInterceptorUtils.convertRmiAccessException(
			...
			try {
				return org.juneframework.remoting.rmi.RmiClientInterceptorUtils.invokeRemoteMethod(invocation, stub);
			}
			catch (InvocationTargetException ex) {
	...
	/**
	 * Apply the given AOP method invocation to the given {@link org.juneframework.remoting.rmi.RmiInvocationHandler}.
